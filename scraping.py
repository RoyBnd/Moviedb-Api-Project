...
endpoint = f"{api_base_url}{endpoint_path}?api_key={api_key}" # creating the full url 
...
if r.status_code in range(200,299): # staus_code = 200
    data = r.json()
    ...
    if len(results) > 0:
        ...
        for result in results:
            _id = result['id'] # using '_id' and not 'id' because 'id' is in many cases a reserved word in Python
            #print(result['title'],_id) # print movie titles and unique id for each one
...
print(df.head())
df.to_csv(output, index=False) # Saving the df as csv   
